...
public class DrawItemPile extends DrawBlock {
    ...
    public void draw(Building build) {
        Angles.randLenVectors(build.id, drawItemAmount(build), radius, (x, y) -> {
            Draw.rect(item.fullIcon, build.x + x, build.y + y, itemSize, itemSize);
        });
    }

    public int drawItemAmount(Building build){
        return Math.round(build.items.get(item) / (float)build.block.itemCapacity * itemAmount);
    }
}
